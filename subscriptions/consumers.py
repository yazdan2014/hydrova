...
from .models import DispenseLog, DispenseOrder, Machine, Subscription
from .serializers import DispenseLogSerializer, DispenseOrderSerializer, SubscriptionSerializer
...
def update_order_from_machine(message):
    ...
    with transaction.atomic():
        ...
        if event_type == 'dispense.started':
            ...
            if order.status != DispenseOrder.STATUS_COMPLETED:
            ...
            if order.status not in [DispenseOrder.STATUS_FAILED, DispenseOrder.STATUS_CANCELLED, DispenseOrder.STATUS_COMPLETED]:
