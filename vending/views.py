...
class DirectDispenseView(APIView):
    permission_classes = [permissions.AllowAny]
    ...
    def post(self, request, serial):
        ...
        if not machine.online:
            return Response({'detail': 'Machine is offline.'}, status=status.HTTP_409_CONFLICT)
        ...
        command = {
            ...
            'order_id': order.id,
            'liters': str(liters),
            'wants_bottle': bool(request.data.get('wants_bottle', False)),
            'max_duration_seconds': 30,
        }
        send_to_machine(machine.serial, command)
